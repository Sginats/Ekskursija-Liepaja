...
export default class CatcherScene extends Phaser.Scene {
  ...
  create() {
    ...
    if (window.MinigameFX) {
      window.MinigameFX.applyTheme(this, 'nature');
      this._fxParticles = window.MinigameFX.makeParticles(this);
    }
    ...
    children.forEach((item) => {
      ...
      if (dx < 40 && dy < 30) {
        if (item.getData('good')) {
          ...
          if (window.MinigameFX) {
            window.MinigameFX.burst(this._fxParticles, item.x, item.y, 12);
            if (this._comboMultiplier > 2) window.MinigameFX.flash(this, 100, 0x00ff00);
            if (this._combo >= 5) window.MinigameFX.textPop(this, item.x, item.y, `×${this._comboMultiplier}!`);
          }
