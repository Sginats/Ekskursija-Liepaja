...
export default class FlashlightScene extends Phaser.Scene {
  ...
  _onPointerMove(ptr) {
    ...
    if (window.MinigameFX && Math.random() < 0.08) {
      window.MinigameFX.burst(this._fxParticles, ptr.x, ptr.y, 1);
    }
  }
  ...
  _finish(success) {
    ...
    if (success && window.MinigameFX) {
      window.MinigameFX.burst(this._fxParticles, this._flashX, this._flashY, 25);
      window.MinigameFX.flash(this, 300, 0x00ff00);
    }
